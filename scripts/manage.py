...
def main() -> None:
    ...
    parser.add_argument(
        "command",
        choices=["init-budget", "budget", "run-pending", "reconcile", "metrics"],
    )
    ...
    if args.command == "init-budget":
        if not args.confirm_starting_usage or not settings.budget_verified:
            parser.error(
                "Verify billing first, set AVERIS_BUDGET_VERIFIED=true and confirm starting usage."
            )
        ...
        with db.session() as session, session.begin():
            if session.get(Budget, 1) is not None:
                parser.error(
                    "Budget already initialized. This command never resets spending."
                )
            session.add(
                Budget(
                    id=1,
        ...
        with db.session() as session:
            ...
            if ledger is None:
                ...
                print(
                    {
                        "prior_usd": ledger.prior_spend / 1_000_000,
        ...
        if args.command == "metrics":
            ...
            if not settings.live_enabled or not settings.budget_verified:
                parser.error(
                    "Live processing is disabled. Configure and verify the shared allowance first."
                )
            with db.session() as session:
            ...
            for run_id in run_ids:
                print(
                    {"run_id": run_id, "status": processor.execute(run_id)}, flush=True
                )
