...
def validate(path: Path, root: Path) -> dict[str, Prediction]:
    submission = TypeAdapter(dict[str, Prediction]).validate_json(
...
def export_cases(snapshot: Path, output: Path, diagnostics: Path, root: Path) -> bool:
    ...
    cases = TypeAdapter(dict[str, CaseView]).validate_json(
    ...
    mapped = [
        case.model_copy(update={"id": source_id}) for source_id, case in cases.items()
    ]
    expected = [email["email_id"] for email in load_emails(root)]
    result = export_submission(mapped, expected)
    count = len(expected)
    automatic = sum(
        not item.reviewer_assisted and not item.blockers
        for item in result.diagnostics.values()
    )
    assisted = sum(
        item.reviewer_assisted and not item.blockers
        for item in result.diagnostics.values()
    )
    sidecar = {
    ...
    if output.exists() or diagnostics.exists():
        raise ValueError(
            "Choose new output paths; existing evaluation artifacts are never overwritten"
        )
    ...
    output.write_text(
        json.dumps(result.official_payload(), indent=2) + "\n", encoding="utf-8"
    )
    return True
...
def main() -> None:
    ...
    if args.command == "export":
        ...
        if not export_cases(args.submission, args.output, args.diagnostics, args.data):
            parser.exit(
                2,
                "Export blocked; diagnostics written, no official submission created.\n",
            )
            ...
            json.dumps(
                {
                    ...
                    "formats": dict(Counter(p.suffix for p in attachments)),
                },
                indent=2,
            )
        )
    ...
    subprocess.run(
        [sys.executable, str(scorer), str(args.submission.resolve()), "--json"],
        check=True,
    )
