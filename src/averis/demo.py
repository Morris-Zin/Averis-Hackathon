...
VALUES = {
    ...
    "container_count": "3", "gross_weight_kg": "Gross weight (kg): 24000",
}


def evidence(document_id: str, role: str, reference: str, values: dict[str, str]) -> DocumentEvidence:
    blocks = [EvidenceBlock(id="header", text=f"{'Shipping Instruction' if role == 'SI' else 'Draft Bill of Lading'}\nShipment ID: {reference}",
                            locations=[Location(kind="text", line_start=1, line_end=2)])]
    line = 4
    for name in FIELDS:
        value = values[name]
        text = value if name == "gross_weight_kg" else f"{name.replace('_', ' ').title()}: {value}"
        end = line + text.count("\n")
        blocks.append(EvidenceBlock(id=name, text=text, locations=[Location(kind="text", line_start=line, line_end=end)]))
        line = end + 2
    return DocumentEvidence(document_id=document_id, blocks=blocks, parser_version="controlled-demo-v1")
...
def seed(session: Session, workspace: Workspace, storage: Storage) -> None:
    ...
    for index, (subject, scenario) in enumerate(scenarios):
        case_id = uid()
        category = "INVOICE_QUERY" if scenario == "invoice" else "SI_REQUEST" if scenario == "si" else "GENERAL" if scenario == "general" else "SPAM" if scenario == "spam" else "BL_COMPARISON"
        now = utcnow() - timedelta(minutes=index * 23)
        view = CaseView(id=case_id, subject=subject, sender="shipping@meridian.example",
            body="Please review the attached shipping instruction and draft bill of lading before release. Thank you."
                if category == "BL_COMPARISON" else subject,
            received_at=now.isoformat(), revision=1, input_revision=1,
            classification=Classification(suggested=category, confidence=.61 if scenario == "uncertain" else 1,
                probabilities={category: .61 if scenario == "uncertain" else 1, **({"GENERAL": .39} if scenario == "uncertain" else {})},
                accepted=None if scenario == "uncertain" else category, source="fixture", model="controlled-demo"),
            processing="completed", stage="saved_demo", workflow="open", assignee="John Tan",
            review_reasons=["Check category"] if scenario == "uncertain" else [], attachments=[],
            history=[AuditEntry(at=now.isoformat(), actor="Demo setup", action="created",
        ...
        if category == "BL_COMPARISON":
            for role in ("SI", "BL"):
                ...
                session.add(Document(id=doc_id, case_id=case_id, workspace_id=workspace.id,
                    filename=filename, object_key=key, sha256=digest, size=len(content)))
                view.attachments.append(AttachmentView(id=doc_id, filename=filename, role=role, evidence=doc))
                readings[role] = {name: reading_from_evidence(name, doc, [name]) for name in FIELDS}
            if scenario != "uncertain":
                view.report = compare(readings["SI"], readings["BL"], 1, True)
                if scenario == "missing":
                    view.review_reasons = ["Missing gross weight"]
        session.add(Case(id=case_id, workspace_id=workspace.id, revision=1, input_revision=1, state=view.model_dump(mode="json")))


def controlled_revision(session: Session, row: Case, view: CaseView, storage: Storage) -> None:
    ...
    session.add(Document(id=doc_id, case_id=row.id, workspace_id=row.workspace_id,
                         filename=name, object_key=key, sha256=digest, size=len(content)))
    old.role = "unknown"
    old.superseded = True
    view.attachments.append(AttachmentView(id=doc_id, filename=name, role="BL", evidence=doc))
    view.workflow = "open"
